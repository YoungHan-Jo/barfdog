...
비밀번호, 비밀번호 확인 필수 값 아님(sns 로그인 회원가입시 사용가능) +
...
[[resources-resources-isNeedToSetPassword]]
=== 비밀번호 설정해야하는 유저인지 확인

`GET` 요청을 사용해서 비밀번호 설정해야하는 유저인지 확인 할 수 있다. +

비밀번호 설정해야하는 유저일 경우, 결과값: true -> 회원정보 수정하기 전, sns 유저 비밀번호 설정 api로 이동 +
비밀번호 설정하지 않아도 될 유저일 경우 경우, 결과값: false -> 회원정보 수정하기 api로 이동 +

operation::check_needToSetPassword[snippets='curl-request,request-headers,request-body,response-headers,response-body,response-fields,links']


[[resources-update-password-snsMember]]
=== 회원정보 수정하기 전 sns 유저 비밀번호 설정

`POST` 요청을 사용해서 회원정보 수정 하기 전 sns 유저 비밀번호를 설정 할 수 있다. +

비밀번호 설정해야하는 유저가 이 api를 사용한 후 부터는 +
비밀번호 설정하지 않아도 될 유저로 바뀌어서 +
비밀번호 설정해야하는 유저인지 확인 api 결과값으로 false 가 나옴 +

operation::update_password_snsMember[snippets='curl-request,request-headers,request-body,request-fields,response-headers,response-body,response-fields,links']
...
`POST` 요청을 사용하여 페이징으로 조건에 맞는 주문 리스트를 상품단위로 조회할 수 있다. +
...
`POST` 요청을 사용하여 페이징으로 조건에 맞는 주문 리스트를 주문단위로 조회할 수 있다. +
...
일반 주문 취소요청 반려 시, 주문상태 : DELIVERY_READY +
구독 주문 취소요청 반려 시, 주문상태 : PRODUCING +
...
일반 주문(order)과 그에 해당하는 모든 주문상품(orderItem)리스트에 대해서 조회할 수 있다 +
...
=== 반품/교환 관련 일반 주문 하나 조회

`GET` 요청을 사용하여 반품/교환 관련 일반 주문 하나를 조회할 수 있다. +

주문상품(orderItem) id로 조회 하기 +
주문상품 id 에 해당하는 주문상품 정보 하나와, 주문상품을 품고 있는 주문(order)에 관한 정보를 조회 할 수 있다. +
...
묶음배송이 아닐 경우 배송 상태: 주문취소 +
묶음배송일 경우 배송 null +

주문 상태 : CANCEL_ORDER_BUYER +
취소요청일/컨펌일 설정 +
...
주문한상품(orderItem)을 포함하고 있는 주문을 통째로 취소시킴 +
...
묶음배송이 아닐 경우 배송 상태: 주문취소 +
묶음배송일 경우 배송 정보 제거 : null +

주문 상태 : SELLING_CANCEL +
주문 취소요청일/컨펌일 설정 +
주문 취소 사유 +

아이템 재고 복구 +
주문한 아이템 상태: SELLING_CANCEL +
주문 아이템 취소요청일/컨펌일 설정 +
주문 아이템 취소 사유 +
쿠폰 복구 +
옵션 재고 복구 +
...
=== 일반 주문 반품요청을 구매자 귀책으로 컨펌 처리(상품단위)

`POST` 요청을 사용하여 일반 주문 반품요청을 구매자 귀책으로 컨펌 처리할 수 있다. +

주문한 상품(orderItem) 상태 : RETURN_DONE_SELLER +
쿠폰은 환불됨 +

적립금, 환불금액(배송비계산)은 CS처리 해야함 +
...
=== 일반 주문 반품요청을 판매자 귀책으로 컨펌 처리(상품단위)
...
=== 일반 주문 반품요청 거절 처리(상품단위)

`POST` 요청을 사용하여 일반 주문 반품요청 거절 처리할 수 있다. +

주문한 아이템(orderItem)을 포함한 주문(order)의 상태와 동일한 상태로 변경 +
...
=== 일반 주문 교환요청을 구매자 귀책으로 컨펌 처리(상품단위)

`POST` 요청을 사용하여 일반 주문 교환요청을 구매자 귀책으로 컨펌 처리할 수 있다. +

주문한 상품(orderItem) 상태 : EXCHANGE_DONE_BUYER +
...
=== 일반 주문 교환요청을 판매자 귀책으로 컨펌 처리(상품단위)

`POST` 요청을 사용하여 일반 주문 교환요청을 판매자 귀책으로 컨펌 처리할 수 있다. +

주문한 상품(orderItem) 상태 : EXCHANGE_DONE_SELLER +
...
=== 일반 주문 주문하기(결제 전 주문 정보 저장)

`POST` 요청을 사용하여 일반 상품을 주문할 수 있다. +
...
- 주문한 아이템(orderItem) 정보 생성 +

- 선택한 옵션(selectOption) 정보 생성, 옵션 재고 차감 +
...
- ㅁ첫결제일 경우ㅁ +
- 회원 첫결제 여부 true로 변환 +
- 추천한 친구 적립금, 적립금 내역 추가 +
- sms,이메일 수신 동의 true 일 경우 적립금 추가, 적립금 내역 추가 +

- ㅁ배송ㅁ +
- 묶음배송 아닐경우에만 배송상태 PAYMENT_DONE +

- ㅁ주문한 상품(orderItem)ㅁ +
- 장바구니에서 제거 +
- 주문한 상품 상태 PAYMENT_DONE으로 변경, writeableReview : true로 변경 +
...
[[resources-cancelPayment-generalOrder]]
=== 일반 주문 결제 취소

`POST` 요청을 사용하여 일반 주문 결제취소 상태를 반영할 수 있다.
...
operation::cancelPayment_generalOrder[snippets='curl-request,path-parameters,request-headers,request-body,response-headers,response-body,response-fields,links']
...
새로운 배송정보 생성 : BEFORE_PAYMENT +
배송 예정일 저장(첫 결제는 프론트에서 넘어오는 날짜로 저장) +
...
구독 상태 : SUBSCRIBING +
카드 추가 +
다음 배송일 계산해서 설정(FULL : 14일 후/ HALF,TOPPING : 28일 후) +
다음결제일 계산해서 설정(FULL : 14일 후/ HALF,TOPPING : 28일 후) +

ㅁ 다음 회차 주문 생성 ㅁ +
다음회차 배송 : BEFORE_PAYMENT, 다음 배송예정일 계산 +
주문 상태 : RESERVED_PAYMENT +
예약주문 merchantUid -> subscribe에 추가 +
...
[[resources-cancelPayment-subscribeOrder]]
=== 구독 주문 결제 취소 처리

`POST` 요청을 사용하여 구독 주문 결제 취소 상태를 반영 할 수 있다. +
...
operation::cancelPayment_subscribeOrder[snippets='curl-request,path-parameters,request-headers,request-body,response-headers,response-body,response-fields,links']
...
order status가 PRODUCING 일 경우 구독주문 취소 요청 처리 (상태만 CANCEL_REQUEST로 변경) +
...
operation::cancelRequest_subscribe[snippets='curl-request,path-parameters,request-headers,request-body,request-fields,response-headers,response-body,response-fields,links']
...
`POST` 요청을 사용하여 일반 주문을 취소 요청할 수 있다. +
order status가 DELIVERY_READY or PRODUCING 일 경우 주문전체 취소 요청 처리 (상태만 CANCEL_REQUEST로 변경) +
order status가 PAYMENT_DONE 일 경우 즉시 카드/적립금/쿠폰 환불처리 됨 +
일반 주문 취소 +
회원 누적 구매 차감 +
회원 적립금 복구 +
적립금 복구 내역 생성 +
...
주문(order) 상태 : CANCEL_ORDER_BUYER +
취소요청일/완료일 : 현재 +

아이템 재고 복구 +
주문한 아이템(orderItem) 상태: CANCEL_ORDER_BUYER +
취소요청일/완료일 : 현재 +
쿠폰 복구 +
옵션 재고 복구 +
...
operation::cancelRequest_general[snippets='curl-request,path-parameters,request-headers,request-body,request-fields,response-headers,response-body,response-fields,links']
...
주문상품(orderItem) 상태 : CONFIRM +
주문상품(orderItem) 적립예정금 적립 완료 +

주문상품을 포함한 주문의 모든 주문상품(orderItem)이 CONFIRM 이면 주문(order)도 CONFIRM +
컨펌시간 설정 +
...
`POST` 요청을 사용하여 다음 회차 구독에 쿠폰을 적용할 수 있다. +

기존에 적용된 쿠폰 없는 경우 : 새로운 쿠폰 적용 +
기존에 적용된 쿠폰 있는 경우 : 기존 쿠폰 살리고 새로운 쿠폰 적용 +

쿠폰 적용 후 아임포트api 기존 예약 취소 후 새로운 정보로 예약 +
...
변경 전 구독 정보 이전구독정보(beforeSubscribe)로 저장 +
강아지 설문 기존 그램에서 변경한 그램으로 그램 수정 +
새로운 구독상품 가격으로 쿠폰할인, 등급할인 적용해서 아임포트api 기존 예약 취소 후 새로운 정보로 예약 +
...
`POST` 요청을 사용하여 다음번 구독을 주/회 단위로 건너 뛰기 할 수 있다. +
...
`POST` 요청을 사용하여 다음회차부터 구독을 중단 할 수 있다. +
